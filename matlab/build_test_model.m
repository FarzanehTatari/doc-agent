...
%   The script is idempotent — re-running deletes the old files first.
    ...
    % ---- Stateflow chart: StatusMonitor (exercises the SF walker) -----
    % A simple 3-state mode machine: OK -> DEGRADED -> INVALID with a
    % recovery path. The chart is standalone (no input/output ports) and
    % never simulated — it exists purely so extract_slx.m has charts and
    % states and transitions to walk.
    try
        addStatusMonitor(modelName);
    catch ME
        warning('build_test_model:NoStateflow', ...
            'Skipping Stateflow chart (toolbox unavailable?): %s', ME.message);
    end
...
% ====================================================================
% addStatusMonitor — wires a small Stateflow chart into the model.
% ====================================================================
function addStatusMonitor(modelName)
    chartPath = sprintf('%s/StatusMonitor', modelName);
    add_block('sflib/Chart', chartPath, 'Position', [40 280 240 460]);

    % Grab the chart object via the Stateflow API
    sfRoot   = sfroot;
    charts   = sfRoot.find('-isa', 'Stateflow.Chart', 'Path', chartPath);
    assert(~isempty(charts), 'Stateflow.Chart not found at %s', chartPath);
    chart    = charts(1);

    % Local data so the actions/conditions reference defined symbols.
    % All Local scope — no input/output ports needed on the chart block.
    addData(chart, 'status_local',     'int8',    'Local');
    addData(chart, 'wheels_partial',   'boolean', 'Local');
    addData(chart, 'wheels_all_bad',   'boolean', 'Local');
    addData(chart, 'wheels_recovered', 'boolean', 'Local');

    % --- States ----------------------------------------------------
    sOK = Stateflow.State(chart);
    sOK.Position    = [40  60 130 60];
    sOK.LabelString = sprintf('OK\nentry: status_local = 0;');

    sDEG = Stateflow.State(chart);
    sDEG.Position    = [220 60 140 60];
    sDEG.LabelString = sprintf('DEGRADED\nentry: status_local = 1;');

    sINV = Stateflow.State(chart);
    sINV.Position    = [220 170 140 60];
    sINV.LabelString = sprintf( ...
        'INVALID\nentry: status_local = 2;\nduring: status_local = 2;');

    % --- Default transition into OK --------------------------------
    dt = Stateflow.Transition(chart);
    dt.Destination          = sOK;
    dt.DestinationOClock    = 9;            % enter the OK state from its left
    dt.SourceEndPoint       = [10 90];
    dt.MidPoint             = [25 90];
    ...
    t1.Source       = sOK;
    t1.Destination  = sDEG;
    t1.LabelString  = '[wheels_partial]';

    t2 = Stateflow.Transition(chart);
    t2.Source       = sDEG;
    t2.Destination  = sINV;
    t2.LabelString  = '[wheels_all_bad]';

    t3 = Stateflow.Transition(chart);
    t3.Source       = sINV;
    t3.Destination  = sOK;
    t3.LabelString  = '[wheels_recovered]';
end


function addData(chart, name, dataType, scope)
